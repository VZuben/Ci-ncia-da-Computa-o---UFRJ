...
//                        posicao, contRepetidas == null ? 1 : contRepetidas + 1);
    ...
    public boolean possuiItemColado(int posicao) {
        if(posicao > 0 && posicao <= getTamanho() && this.figurinhasColadas.get(posicao) != null) return true;
...
//        return contador;
    ...
    public void autoCompletar() {
        if(this.quantFigurinhasColadas != 0) {
            for (int i = 1; i < this.figurinhasColadas.size(); i++) {
                Figurinha figurinha = this.repositorio.getTodasAsFigurinhas().get(i-1);
                if (!(possuiItemColado(i)) && this.figurinhasColadas.get(i) == null && figurinha != null) {
                    this.figurinhasColadas.set(i, figurinha);
                    this.quantFigurinhasColadas++;
                }
            }
        }
