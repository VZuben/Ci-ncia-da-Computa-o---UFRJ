...
void * leitor (void * arg) {
    ...
    while(1) {
        printf("L[%d] quer ler\n", *id);
        sem_wait(&leit);
        printf("L[%d] consegue ler\n", *id);
        sem_wait(&em_l);
        l++;
        if(l==1){
            sem_wait(&escr);
            printf("L[%d] está bloqueando nova thread escritora\n", *id);
        }
        sem_post(&em_l);                      
        sem_post(&leit);
        ...
        l--; 
        if(l==0){
            printf("L[%d] está desbloqueando nova thread escritora\n", *id);
            sem_post(&escr);
        } 
        sem_post(&em_l);
        sleep(1);
...
void * escritor (void * arg) {
    ...
    while(1) {
        ...
        if(e==1){
            sem_wait(&leit);
            printf("E[%d] está bloqueando novas threads leitoras\n", *id);
        }
        sem_post(&em_e);
        sem_wait(&escr);     
        ...
        e--; 
        if(e==0){
            printf("E[%d] está desbloqueando novas threads leitoras\n", *id);
            sem_post(&leit);
        } 
        sem_post(&em_e);
        sleep(1);
