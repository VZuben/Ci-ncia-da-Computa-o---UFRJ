...
/* Codigo: Função que  realiza a Multiplicacao de matriz-matriz (considerando apenas matrizes quadradas) de forma sequêncial e concorrente*/
...
int main(int argc, char* argv[]) {
   ...
   //multiplicacao da matriz A pela matriz B de forma sequencial
   GET_TIME(inicio);
   mult_mat_seq(mat_A, mat_B, saida_seq, dim);
   GET_TIME(fim)   
   tempo_seq = fim - inicio;

   //multiplicacao da matriz A pela matriz B de forma concorrente
   GET_TIME(inicio);
   ...
   //testando se a saida sequencial é igual a saida concorrente
   if (corretude(saida_seq, saida_conc, dim) != 0) {
      printf("As matrizes de saída não são iguais\n");
      return 4;
   } else printf("As matrizes de saída são iguais\n");

   //printando os tempos de execução das multiplicações e a aceleração.
