...
int vetor_ao_quadrado[QNT_ELEMENTOS];
...
void * ao_quadrado (void * arg) {
    ...
    if (ident == 1) {
        for(int i=0; i<(QNT_ELEMENTOS/2); i++) {
           vetor_ao_quadrado[i] = vetor[i]*vetor[i];
        }
...
int main(void) {
    ...
    //checa se os 10.000 elementos realmente foram elevados ao quadrado
    for(int i=0; i<QNT_ELEMENTOS; i++) {
           if (vetor_ao_quadrado[i] != vetor[i]*vetor[i]) {
               printf("Deu errado!\n");
               return -1;
           }
    }
